...
sub _init{
	...
	if($self->stash->{list_table} =~ /dtbl_subscribe_([\s\S]+)/){
...
sub send_letter_cron{
	...
	for my $i (1..5) {
		for my $user ($self->dbi->query(qq/
			SELECT `dtbl_subscribe_users`.`name` AS `email`,
					`data_subscribe`.`subject`, `data_subscribe`.`text`,
					`dtbl_subscribe_stat`.`ID` 
			FROM 
				`dtbl_subscribe_stat`
			LEFT JOIN
				`dtbl_subscribe_users`
				ON `dtbl_subscribe_stat`.`id_user`=`dtbl_subscribe_users`.`ID`
			LEFT JOIN 
				`data_subscribe`
				ON `dtbl_subscribe_stat`.`id_data_subscribe`=`data_subscribe`.`ID`
			WHERE
				`dtbl_subscribe_stat`.`send_date`='0000-00-00 00:00:00'
			/)->hashes){
			
			
			my $email_body = 	$self->render_mail(
				user 		=> $user,	
				template	=> "Subscribe/letter" 
			);
			...
			$self->dbi->dbh->do("UPDATE `dtbl_subscribe_stat` SET `send_date`=NOW() WHERE `ID`=?", undef, $user->{ID});
		}
	}
}
...
sub send_letter{
	...
	if ($self->getArraySQL( from => $self->stash->{list_table}, where => $self->stash->{index}, stash => 'anketa')) {
		if($self->stash->{anketa}->{'send'}){
			$self->admin_msg_errors( "Рассылка уже запущена ... " );
		} else {
			if (my $users = $self->dbi->query("SELECT `ID` FROM `$DB_USERS` WHERE 1")->flat){
		...
		$self->admin_msg_errors("Выбирете рассылку, к которой отниситься данное письмо");
	}
...
sub list_items{
	...
	if($list_table eq 'data_subscribe'){
		$self->stash->{listfield_buttons} = [qw(delete edit send)];
	}
