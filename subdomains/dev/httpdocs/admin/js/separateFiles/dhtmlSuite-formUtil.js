...
// Array of elements to parse	
	...
	var td = formRef.getElementsByTagName('TD');
	...
	for(var no=0;no<vElements.length;no++){
		...
		var elind = "";
		for(var no1=0;no1<td.length;no1++){	if (td[no1].id == '_'+vElements[no].name) elind = td[no1]; }
...
// {{{ isFormValid()
    ...
    *@public
	...
	for(var no in this.formElements){
		if(!this.formElements[no].result && no != 'toQueryString' && no != 'unite'){		
		return false;
		}
	}	
	return true;	
