	...
	************************************************************************************************************/	
	...
	function moveThumbnails()
	{
		...
		if(this.id=='DHTMLgoodies_leftArrow'){
			...
			var showArrow = true;
			if(DHTMLgoodies_thumbTotalWidth + left - DHTMLgoodies_slideWidth <= DHTMLgoodies_viewableWidth-30)showArrow = false;
	...
	function showPreview(imagePath,inputObj,orientation,title)
	{
		document.getElementById("DHTMLgoodies_largeImageAlt").innerHTML = title;
...
//			if(DHTMLgoodies_currentActiveImage==inputObj.getElementsByTagName('IMG')[0])return;
		...
		moveOpacity(opacitySteps*-1,currentUnqiueOpacityId,orientation);
	}
	...
	function moveOpacity(speed,uniqueId,orientation)
	{
		...
		if(DHTMLgoodies_currentOpacity<=5 && speed<0){
			...
			DHTMLgoodies_largeImage.src = DHTMLgoodies_imageToShow;
			if (orientation) {
				DHTMLgoodies_largeImage.style.width = 200 + 'px';
			} else {
				DHTMLgoodies_largeImage.style.height = 200  + 'px';
			}
		...
		if(uniqueId==currentUnqiueOpacityId)setTimeout('moveOpacity(' + speed + ',' + uniqueId + ',' + orientation + ')',opacitySpeed);		

	}
	function set_link() {
		if (DHTMLgoodies_imageToShow) {
			opener.OnUploadCompleted(101, DHTMLgoodies_imageToShow, document.getElementById("DHTMLgoodies_largeImageAlt").innerHTML, 'Выбрана новая картинка') ;
//			opener.document.getElementById("txtUrl").value = DHTMLgoodies_imageToShow;
//			opener.document.getElementById("txtAlt").value = document.getElementById("DHTMLgoodies_largeImageAlt").innerHTML;
			window.close();
		}
	}