...
sub register {
	...
	$app->helper(
		file_save_pict => sub {
			...
			my ($pict_saved, $type_file);

			#($params{folder}, $pict_saved, $type_file) = $self->file_check_free_name($ENV{'DOCUMENT_ROOT'}.$params{folder}.$params{filename});
			(undef, $pict_saved, $type_file) = $self->file_save_from_tmp( filename => $params{filename}, to => $params{folder}.$params{filename} );
