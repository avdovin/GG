				...
				if ($self->dbi->exists_keys(from => $lkey_dop_table, lkey => $k) and ($self->app->sysuser->access->{lkey}->{$k}->{r} or ($$lkey_settings{sys} || $self->app->sysuser->sys))) {
					...
					if ($lkey->{type} eq "tlist") {
						if (!$lkey->{list_from_key}) {
							$self->stash->{"table_list_dp_$lfield"}->{table_from} .= " LEFT JOIN ".$lkey->{settings}->{list}." AS `tb".$sch."` ON ${lfield}_dop_table.`$k` = tb".$sch.".`ID`";
...
#					}
	...
	$app->helper(
		lfield_folder => sub {
			...
			unless($folder){
				$folder = $self->lkey(name => $params{lfield} )->{settings}->{folder};
				
				if($self->lkey(name => $params{lfield} )->{settings}->{mini}){
					my $mini = $self->lkey(name => $params{lfield} )->{settings}->{mini};
	...
	$app->helper(
		restore_doptable => sub {
			...
			$self->stash->{index} = $self->stash->{ $self->app->lkeys->{$lfield}->{settings}->{table_svf} };
