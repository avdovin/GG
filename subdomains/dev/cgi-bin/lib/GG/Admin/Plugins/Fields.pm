		...
		field_url_for => sub {
			...
			sub _draw {
			  ...
			  foreach my $node (@$routes) {
			  	...
			  	$options .= "<option data-alias='$route_alias' value='$route_name' ".( $value eq $route_name ? " selected='true' " : "").">$route_admin_name</option>";
			  }
			  return $options;
			}
	...
	$app->helper(
		field_delete_file => sub {
			...
			if(my $item = $self->getArraySQL( 	from 	=> $params{table},
				...
				my $document_root = $self->stash->{document_root};
	...
	$app->helper(
		lists_select_search => sub {
			...
			if(my $rules = $self->param('rules')){
				my $list_table = $self->lkey(name => $lfield, setting => 'list' );
				...
				if(my $rules = $self->param('rules')){
					...
					my ($field_rules_f1, $field_rules_f2) = split(/=/, $field_rules);
			
					if ($field_rules_f2 eq "index") {
						$where .= " ".$self->lkey(name => $field_rules_f1, setting => 'rules_where' );
						$where .= " AND `$field_rules_f1`='".$self->stash->{ $field_rules_f2 }."' ";
					
					} else {
			
						$select = "`".$self->lkey(name => $field_rules_f2, setting => 'list' )."`.`ID`,`".$self->lkey(name => $field_rules_f2, setting => 'list' )."`.`name`";
						$where  = "`$from`.`ID`='".$self->stash->{ $field_rules_f2 }."' GROUP BY `$field_rules_f2`";
						$from   = "`$from` INNER JOIN ".$self->lkey(name => $field_rules_f2, setting => 'list' )." ON `$from`.`$field_rules_f2`=`".$self->lkey(name => $field_rules_f2, setting => 'list' )."`.`ID`";			
					}		
				} else {
...
#				}
				...
				if(my $table = $self->lkey(name => $lfield, setting => 'list' )){
					my $where  = " 1 ";
					$where	.= " ".$self->lkey(name => $lfield, setting => 'where' )." " if $self->lkey(name => $lfield, setting => 'where' );
