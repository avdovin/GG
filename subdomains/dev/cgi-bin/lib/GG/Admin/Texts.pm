...
sub body{
	...
	given ($do){
		...
		when('upload') 					{ 
			...
			$self->file_download( path => $item->{folder}.$item->{docfile});
		}
...
sub save{
	...
	if( $self->save_info( table => $self->stash->{list_table}) ){
		...
		if($self->send_params->{docfile}){
			...
			my $size = -s $ENV{DOCUMENT_ROOT}.$folder.$docfile_saved || 0;
			
			$self->save_info(send_params => 0, table => $self->stash->{list_table}, field_values => {size => $size, docfile => $docfile_saved, folder => $folder, type_file => $type_file} );
		}
