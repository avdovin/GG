...
#	}
	...
	if (exists $self->{REQUEST}->{'init'})
	{
		...
		%{$self->{RES}->{'options'}} = (
			...
			#'tmbUrl'		=>  $self->{CONF}->{URL}.$DIRECTORY_SEPARATOR,
...
sub _paste{
	...
	foreach my $target (@$targets){
		...
		if($cut){
			__remove_node($src.$DIRECTORY_SEPARATOR.$name);
			push @$removed, $target;	
		}
	}	
...
sub _abs_path{
	...
	return $path eq $DIRECTORY_SEPARATOR ? $self->{CONF}{'root'} : $self->{CONF}{'root'}.$DIRECTORY_SEPARATOR.$path;
}
...
sub _rel_path{
	...
	return $path eq $self->{CONF}{'root'} ? '' : substr($path, length($self->{CONF}{'root'})+1);
}
