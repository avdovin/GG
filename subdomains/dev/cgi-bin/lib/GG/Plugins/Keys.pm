...
#			}
			...
			$self->validator( controller => $params{controller} ) if $params{validator};
		}
	);
	...
	$app->helper(
		lkey => sub {
			...
			if($self->app->lkeys->{$controller}->{$lkeyName}){
				if(my $list_table = $self->stash->{list_table}){
					
					my $lkey = ($lkeys->{$controller}->{tbl}->{$list_table} && 
								$lkeys->{$controller}->{tbl}->{$list_table}->{$lkeyName}) ?
								$lkeys->{$controller}->{tbl}->{$list_table}->{$lkeyName} :  
...
sub _parseLkeys{
	...
	foreach my $dbKey (@$lkeys){
		...
		if($lkey->{object} eq 'lkey'){
			...
			if($lkey->{tbl}){
				$app->lkeys->{$controller}->{tbl}->{$lkey->{tbl}}->{$lkey->{lkey}} = $lkey
