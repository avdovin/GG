...
sub register {
	...
	$app->helper(
		banner_def => sub {
			...
			$params{template} ||= 'Banners/manywithdelimiter';
			$params{delimiter} ||= "";
			...
			$params{page} = $self->lang.":main:".$self->stash->{info}->{ID};
	
			if (!$params{place}) {
				return ""; # если не определено место показа, то ничего не выводится вообще
			} else {
		    ...
		    my $total_banner = $params{noblock} ? 1 : $params{cols} * $params{rows};    # Общее количество баннеров
		    ...
		    my $where  = "`view` = 1";
			   ...
			   $where .= " ORDER BY `rating`,RAND() LIMIT 0,$total_banner";
