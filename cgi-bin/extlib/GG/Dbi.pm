...
sub getKeysSQL {
  ...
  if (!$params{from}) {
    Carp::croak(
      "Функция getKeysSQL. Отсутствует параметр FROM"
    );
  }
...
sub exists_keys {
  ...
  if (!$params{from}) {
    Carp::croak(
      "Функция exists_keys. Отсутствует параметр FROM"
    );
  }
...
sub getCountCol {
  ...
  if (!$params{from}) {
    Carp::croak(
      "Функция getCountCol. Отсутствует параметр FROM"
    );
  }
...
sub upsert {
  ...
  my $sql = sprintf "INSERT INTO %s (%s) VALUES (%s)", $table,
    join(",", map {"`$_`"} @fields), join(",", ("?") x @fields);
...
sub insert {
  ...
  my $sql = sprintf "$insType INTO %s (%s) VALUES (%s)", $table,
    join(",", map {"`$_`"} @fields), join(",", ("?") x @fields);
...
sub insert_hash {
  ...
  $field_values->{'updated_at'} = undef
    if ($field_values->{'updated_at'} eq '0000-00-00 00:00:00'
    or $field_values->{'updated_at'} eq '0000-00-00');
...
sub update {
  ...
  my @fields = sort keys %$field_values;
  my @values = @{$field_values}{@fields};
  my $sql    = sprintf "UPDATE `%s` SET %s WHERE $where", $table,
    join(",", map {"`$_`=?"} @fields);
