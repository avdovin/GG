...
sub register {
  ...
  $app->routes->route("/captcha/code.jpg")->to(
    cb => sub {
      my $self = shift;
      ...
      $self->render(data => $self->captcha($code));
  ...
  $app->helper(
    check_captcha => sub {
      ...
      $self->dbi->dbh->do(
        "DELETE FROM `captcha` WHERE TO_DAYS(NOW())-TO_DAYS(`time`)>1");

      if (my ($code)
        = $self->dbi->query("SELECT code FROM `captcha` WHERE `code`='$code'")
        ->flat)
      {
        $self->dbi->dbh->do(qq~DELETE FROM `captcha` WHERE `code`=?~,
          undef, $code);
        return $code;
      }
      return;
  ...
  $app->renderer->add_helper(
    captcha => sub {
      ...
      my $img = Image::Magick->new(
        size => $captcha_conf->{width} . "x" . $captcha_conf->{height});
      my $x;
      $x = $img->Read($self->app->home->rel_dir('data/captcha.jpg'));
