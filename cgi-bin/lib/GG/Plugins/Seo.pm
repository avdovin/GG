...
sub register {
  ...
  $app->helper(
    render_footer => sub {
      my $self = shift;
      my %args = (template => '_footer', @_);

      return $self->render_to_string(%args);
    }
  );
    ...
    render_headers => sub {
      ...
      if ($self->stash->{seo}) {    # && $self->seo_custom_tags ){
        ...
        if (!scalar @{$metaTags->{title}} && $self->stash->{alias}) {
          my $textMetaTags
            = $self->app->dbi->query(
                "SELECT `title`,`keywords`,`description` FROM `texts_main_"
              . $self->lang
              . "` WHERE `alias`='"
              . $self->stash->{alias}
              . "' LIMIT 0,1 ")->hash;
  ...
  $app->helper(
    seo_custom_tags => sub {
      ...
      unless ($found) {
        if (
          my $node = $self->dbi->query(
            "SELECT *, `name` AS `title` FROM `$DataTable` WHERE `url` LIKE '%"
              . $reqUrl
              . "' LIMIT 0,1"
          )->hash
          )
