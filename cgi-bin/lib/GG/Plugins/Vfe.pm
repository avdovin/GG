  ...
  $app->helper(
    vfe_template => sub {
      ...
      my $path = $self->app->home->rel_dir(
        "/templates/Vfe/templates/" . $params{name} . '.html');
...
# Есть ли предыдущии версии? (Для кнопки Undo). Только если администратор.
  ...
  $app->routes->post("/admin/vfe-text-save")->to(
    cb => sub {
      my $self   = shift;
      my %params = @_;
      my $vals   = {error => '',};
      ...
      if ($content && $id) {
        ...
        if (substr($id, 0, 6) eq 'blocks') {
          ...
          if (!ref $e and !$e) {
            $self->app->dbi->update_hash('texts_main_' . $lang,
              {text => $content}, "`ID`='$id'");
          }
          else {
            return $self->render(
              text => "Ошибка при сохранении: " . $e);
          }
        }
  ...
  $app->routes->route("admin/vfe-css-read")->to(
    cb => sub {
      ...
      if (my $data = $self->file_read_data(path => $path)) {
        ...
        $vals->{error}
          = "Невозможно открыть файл со стилями";
        return $self->render(json => $vals);
      }
  ...
  $app->routes->route("admin/vfe-undo")->to(
    cb => sub {
      ...
      if ($revision > 0) {
        my $path = $self->app->home->rel_dir(
          "/templates/Vfe/backups/" . $template . "__" . $revision . ".txt");
  ...
  $app->routes->route("admin/vfe-redo")->to(
    cb => sub {
  ...
  $app->routes->route("admin/vfe-save")->to(
    cb => sub {
      ...
      my $path = $self->app->home->rel_dir(
        "/templates/Vfe/templates/" . $template . '.html');
      ...
      if (my $data = $self->file_read_data(path => $path)) {
        ...
        unless (-e $self->app->home->rel_dir("/templates/Vfe/backups")) {
          unless (
            mkdir($self->app->home->rel_dir("/templates/Vfe/backups"), 0777))
          {
            $vals->{error}
          ...
          unless (
            opendir(DIR, $self->app->home->rel_dir("/templates/Vfe/backups")))
          {
            $vals->{error}
              = "Невозможно открыть папку ревизий. Ошибка: "
              . $!;
