...
sub register {
  ...
  $app->helper(save_userdata => sub {
    ...
    if($c->current_user->{is_anonymous}){
      $c->dbi->update_hash($Anonymous_Table, {data => $encoded_data}, "`cck`='$cck'");
    }else{
      $c->dbi->update_hash($User_Table, {data => $encoded_data}, "`ID`=".$c->current_user->{ID});
    }
  ...
  my $validate_user_cb = sub{
    ...
    if($c->dbi->exists_keys(from => $User_Table, lkey => 'password_digest')){
      my $password_digest = $c->dbi->select($User_Table, 'password_digest', $where)->list;
  ...
  $app->validator->add_check(password_digest_eq => sub {
  ...
  $app->helper(authenticate => sub {
    ...
    if(defined($extradata->{auto_validate})) {
      ...
      $c->dbi->update_hash($Anonymous_Table, {user_id => $uid}, "`cck`='".$c->session($session_key)."'");
      delete $c->stash->{$stash_key};
