...
sub _config {
  ...
  my $hash_table = {
    ...
    'texts_blog_ru' => {
      ...
      route        => 'news_item',
      linktmlp     => '',
      mapfields    => {alias => 'list_item_alias',},
      where        => " AND `viewtext`=1 ",
    },
    'data_catalog_items' => {
      searchfields => [qw(name text)],
  ...
  $app->routes->get('search/result')->to(
    ...
    cb                 => sub {
      ...
      foreach my $table (sort keys %$hash_table) {
        ...
        my ($keyFieldSelect)
          = _buildKeyFieldsSelect($hash_table->{$table}->{primary_key});
...
#$self->get_keys( type => ['lkey'], controller => $hash_table->{$table}->{controller} );
        ...
        $search_str .= $hash_table->{$table}->{where}
          if $hash_table->{$table}->{where};

        if ($search_str) {
          for my $row (
            $self->app->dbi->query(
              "SELECT $keyFieldSelect FROM `$table` WHERE $search_str")->hashes
            )
          {

            $result_search_index[$sch] = {
              table       => $table,
              primary_key => _buildKeyFieldsValue(
                $hash_table->{$table}->{primary_key}, $row
              ),
      ...
      if ($sch && $ksearch) {
        ...
        my $sth_insert
          = $self->app->dbh->prepare(
          "REPLACE INTO `dtbl_search_results` (`idx`,`qsearch`,`table`,`controller`,`tdate`,`primary_key`) values (?, ?, ?, ?, NOW(), ?)"
          );
        foreach my $i (0 .. $#result_search_index) {
          $sth_insert->execute(
            $i + 1,
            $ksearch,
            $result_search_index[$i]->{table},
            $result_search_index[$i]->{controller},
            $result_search_index[$i]->{primary_key}
          );
        }
        $sth_insert->finish();
...
sub print_search_result {
  my $self = shift;
  my %params = (limit => 10, qsearch => '', index => 0, @_);
...
#$params{count} = $self->stash->{total_count} = $self->dbi->getCountCol(from => 'dtbl_search_results', where => $where);
  ...
  my $search_items
    = $self->dbi->query("SELECT * FROM `dtbl_search_results` WHERE $where")
    ->hashes || [];
  ...
  foreach my $row (@$search_items) {
    ...
    foreach my $i (0 .. $#primaryVals) {
      $whereIndex
        .= " AND `"
        . $hash_table->{$table}->{primary_key}->[$i]
        . "`='$primaryVals[$i]' ";
    }

    if (my $node
      = $self->app->dbi->query("SELECT * FROM `$table` WHERE 1 $whereIndex ")
      ->hash)
    {
      ...
      if ($hash_table->{$table}->{route}) {
        $vals->{'link'}
          = $node->{link}
          ? $node->{link}
          : $self->url_for($hash_table->{$table}->{route}, %$node);
      }
        ...
        while ($linktmlp =~ m/\%([\d\w~]+)\%/ig) {
          ...
          if ($type =~ /list/) {
            $node->{$val}
              = $self->VALUES(name => $val, value => $node->{$val}) || '';
          }
          $val = $node->{$val} || "";
