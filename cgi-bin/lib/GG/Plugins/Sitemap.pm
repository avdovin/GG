...
sub register {
  ...
  $app->routes->get('sitemap' => [format => [qw(xml)]])->to( cb => sub{
    my $self   = shift;
    ...
    my $CONFIG = {
      ...
      'catalog_list_by_category' => {
        ...
        'cb'            => sub {
          ...
          return $self->url_for($route, category_alias => $category->{alias}) unless $category->{parent_category_id};
        }
      },
      'catalog_list_by_subcategory'     => {
        ...
        'cb'          => sub {
          ...
          my ($parent_alias) = $self->dbi->query("SELECT `alias` FROM `data_catalog_categorys` WHERE `ID`='$$category{parent_category_id}'")->list;
          return $self->url_for($route, subcategory_alias => $category->{alias}, category_alias => $parent_alias);
        }
      },
    };
