...
sub register {
  ...
  $app->routes->get("/feedback")->to(seo_title_sitename => 1, admin_name => 'Контакты', layout => 'default', alias =>  'feedback', cb => sub{
		my $self   = shift;
		my %params = @_;
  ...
  $app->routes->post("/feedback")->to(alias =>  'feedback', cb => sub{
		my $self   = shift;
		my %params = @_;
