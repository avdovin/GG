...
sub register {
  ...
  $app->helper(
    get_var => sub {
      ...
      if ($_[1]) {
        my %params = (name => '', controller => 'global', raw => 0, @_);
  ...
  $app->helper(
    setVar => sub {
      my $self = shift;
      my %params = (controller => 'global', key => '', value => 0, @_);

      return
        if (!$params{controller}
        or !$params{key}
        or
        !$self->app->vars->{'controller_' . $params{controller}}->{$params{key}}
        );
      ...
      if ($params{controller} ne 'global') {
        if (
          my $program = $self->dbi->query(
            "SELECT `ID` FROM `sys_program` WHERE `key_razdel`='$params{controller}' LIMIT 0,1 "
          )->hash
          )
        {
      ...
      $self->dbi->dbh->do(
        "UPDATE `sys_vars` SET `envvalue`='$params{value}' WHERE `envkey`='$params{key}' AND `id_program`='$programId' "
      );

      $self->app->vars->{'controller_' . $params{controller}}->{$params{key}}
        ->{envvalue} = $params{value};
    }
  );
...
sub _loadVars {
  ...
  unless ($self->app->vars) {
      ...
      )
    {
      ...
      $self->app->vars->{'controller_' . $controller}->{$row->{envkey}}
        = GG::Plugins::Vars::Var->new($row);
    }
  }
}
