  ...
  $r->route('/admin/form_auth')->to(
    ...
    cb => sub {
      ...
      if (
        $self->admin_getUser(
          updateAccess => 1,
          password     => $self->param('authpassword') || ''
        )
        )
      {
...
#$self->app->dbi->query("UPDATE `sys_users` SET cck='' WHERE `login`='$login'") if $resetCck;
        $self->dbi->dbh->do(
          "DELETE FROM `sys_users_session` WHERE `id_user`=? AND `cck`=? ",
          undef,
          $self->app->sysuser->userinfo->{ID},
          $self->app->sysuser->userinfo->{cck}
        );

        $self->cookie('admin_cck', '',
          {path => '/admin', expires => time - 1000});

        $self->cookie('admin_login', '', {path => '/', expires => time - 1000});

        $self->cookie('vfe', '', {path => '/', expires => time - 1000});
      }

      $self->render(json => {content => 'LOGOUT', items => [],});
    }
  );

  my $admin_route = $r->under('/admin')->to(
    %defaults,
    cb => sub {
      my $self = shift;
  ...
  $app->helper(
    admin_getUser => sub {
      my $self = shift;
      my %params = (updateAccess => 0, password => '', @_);

      $params{login}
        ||= $self->param('userlogin') || $self->cookie('admin_login');
      $params{cck} ||= $self->param('cck') || $self->cookie('admin_cck');

      $self->app->sysuser->auth(0);

      if ($params{password})
      {    # есть пароль и нет ключа сессии
        ...
        $self->cookie(
          ...
          {
            path    => '/admin',
            expires => $self->app->sysuser->userinfo->{cck}
            ? time + 360000000
            : time - 1000
          }
        );
        ...
        my $vfe
          = $self->app->sysuser->userinfo->{login}
          && $self->app->sysuser->userinfo->{vfe} ? 1 : ''
          && $self->app->sysuser->userinfo->{cck} ? 1 : '';
        $self->cookie('vfe', $vfe,
          {path => '/', expires => $vfe ? time + 360000000 : time - 1000});


        # Очистка логов
        if (my $days_count
          = $self->app->get_var(name => 'time_log', controller => 'global'))
        {
          $self->app->dbh->do(
            "DELETE FROM `sys_datalogs` WHERE TO_DAYS(NOW())-TO_DAYS(`created_at`)>$days_count"
          );

          #   Очистка юзер хистори
          $self->app->dbh->do(
            "DELETE FROM `sys_history` WHERE TO_DAYS(NOW())-TO_DAYS(`created_at`)>$days_count"
          );
        ...
        $self->app->dbh->do(
          "DELETE FROM `sys_users_session` WHERE TO_DAYS(NOW())-TO_DAYS(`time`)>5"
        );
  ...
  $app->helper(
    admin_checkCCK => sub {
      my $self = shift;
      my %params = (cck => '', login => '', from => 'sys_users', @_);

      if ($self->param('auth')) {
        if (!$params{login}) {
          $self->admin_msg_errors('Отсутствует логин')
            && return;
        }
        if (!$params{password}) {
          $self->admin_msg_errors('Отсутствует пароль')
            && return;
        }
      }
      ...
      unless ($user) {
        $self->admin_msg_errors('Ошибка login и/или пароля')
          && return;
      }

      if (
        my $session = $self->dbi->query(
          "SELECT * FROM `sys_users_session` WHERE `id_user`='$$user{ID}' AND `cck`='$params{cck}'"
        )->hash
        )
      {

        $self->dbi->dbh->do(
          "UPDATE `sys_users_session` SET `time`=NOW() WHERE `id_user`='$$user{ID}' AND `cck`='$params{cck}' "
        );
...
# восстанавливаем настройки из сессии пользователя
        ...
        $self->admin_msg_errors(
          'Сессия прервана. Требуется повторная авторизация'
        );
      }
  ...
  $app->helper(
    admin_checklogin => sub {
      my $self = shift;
      my %params
        = (from => 'sys_users', count => 10, login => '', password => '', @_);
      ...
      my $sql
        = "SELECT *,TIMESTAMPDIFF(SECOND,btime,NOW())AS `bsecs` FROM `$params{from}` WHERE $params{where} LIMIT 0,1";
      my $user = $self->app->dbi->query($sql)->hash;

      unless ($user) {
        $self->save_logs(
          name =>
            'Пожалуйста, введите верные логин и пароль #1',
          comment =>
            "Логин: $params{login}, Пароль: $params{password}",
          event => 'auth'
        );
        $self->admin_msg_errors(
          ' Пожалуйста, введите верные логин и пароль. <br />Помните, оба поля чувствительны к регистру.'
        ) && return;
      }

      if ($user->{count} && ($user->{count} >= $params{count})
        and ($user->{bsecs} <= 3 * 60 * 60))
      { # Проверка на количество неправильных попыток авторизации
        $self->save_logs(
          name =>
            'Попытка подбора пароля. Доступ для данного аккаунта временно ограничен',
          comment =>
            "Логин: $params{login}, Пароль: $params{password}",
          event => 'auth'
        );
        ...
        eval {
          $self->mail(
            ...
            subject =>
              'Попытка подбора пароля на сайте '
              . $self->get_var(
              name       => 'site_name',
              controller => 'global',
              raw        => 1
            ...
            data =>
              "Логин: <b>$params{login}</b> <br />Пароль: <b>$params{password}</b>, IP: <b>$ip</b>",
          );
        };
      ...
      elsif ($user->{bsecs} && $user->{bsecs} >= 3 * 60 * 60)
      { # Прошло больше трех часов с момента неправильной авторизации
        $sql
          = "UPDATE `$params{from}` SET `count`=0,`btime`=NULL,`bip`='$ip' WHERE `login` = '$$self{user}{login}'";
      ...
      if ($params{'password'}
        && !$self->check_password($params{'password'},
          $user->{'password_digest'}))
      {    # Неверный пароль
        $sql
          = "UPDATE `$params{from}` SET `count`=`count`+1,`btime`=NOW(),`bip`='$ip' WHERE `login`='$$user{login}'";
        $self->app->dbi->query($sql);

        my $count = $user->{count} + 1;
        $self->admin_msg_errors(
          " Пожалуйста, введите верные логин и пароль. <br />Помните, оба поля чувствительны к регистру. <br />Попытка ($count из $params{count})"
        );
        $self->save_logs(
          name =>
            "Пожалуйста, введите верные логин и пароль #2. Попытка ($count из $params{count})",
          comment =>
            "Логин: $params{login}, Пароль: $params{password}",
          event => 'auth'
        );
        ...
        $self->dbi->dbh->do(
          "REPLACE `sys_users_session`(cck, time, host, ip, id_user) VALUES (?, NOW(), ?, ?, ?)",
          undef, $cck, $self->host, $ip, $user->{ID}
        );
  ...
  $app->helper(
    save_logs => sub {
      ...
      my $sysuser
        = $self->app->sysuser->auth ? $self->app->sysuser->userinfo : {};
    ...
    admin_msg_success => sub {
      ...
      if ($_[0]) {
        ...
        if (scalar(@$msg) > 0) {
          ...
          return
              (!$no_wrap ? "<div class='message message-success'>" : "")
            . join("<br />", @$msg)
            . (!$no_wrap ? "</div>" : "");
        }
      }
    ...
    admin_msg_errors => sub {
      ...
      if ($_[0]) {
        ...
        if (scalar(@$msg) > 0) {
          my $no_wrap = delete $self->stash->{msg_no_wrap} || 0;
          return
              (!$no_wrap ? "<div class='message message-error'>" : "")
            . join("<br />", @$msg)
            . (!$no_wrap ? "</div>" : "");
        }
      }
    ...
    param_default => sub {
      ...
      if ($_[0]) {
        ...
        $self->stash->{param_default} = '&'
          . join("&",
          map { "$_=" . $self->stash->{param_default_keys}->{$_} }
            keys %{$self->stash->{param_default_keys}});
      }
      else {
