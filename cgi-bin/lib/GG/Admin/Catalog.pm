...
sub _init {
  ...
  if ($self->stash->{list_table} =~ /data_catalog_([\s\S]+)/) {
    $self->app->program->{groupname}
      = $self->app->program->{settings}->{'groupname_' . $1}
      if ($1 && $self->app->program->{settings}->{'groupname_' . $1});
  }
}
...
sub _update_sizes {
  ...
  $self->dbi->dbh->do("DELETE FROM dtbl_catalog_item_sizes WHERE item_id=?",
    undef, $item_id);
  my @sizes = ();
  foreach my $size (split(',', $sizes)) {
    ...
    $self->dbi->dbh->do(
      "REPLACE INTO dtbl_catalog_item_sizes(item_id, size, qnt) VALUES ('$item_id', '$k', '$v')"
    );
  }
...
sub save {
  ...
  if ($self->stash->{list_table} eq 'data_catalog_items'
    && defined $self->send_params->{'sizes'})
  {
    $self->send_params->{'sizes'} = $self->_update_sizes($self->stash->{index},
      $self->send_params->{'sizes'});
  }
