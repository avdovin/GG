...
sub save {
  ...
  $self->send_params->{youtubelink}
    =~ s/(?:http|https|)(?::\/\/|)(?:www.|)(?:youtu\.be\/|youtube\.com(?:\/embed\/|\/v\/|\/watch\?v=|\/ytscreeningroom\?v=|\/feeds\/api\/videos\/|\/user\S*[^\w\-\s]|\S*[^\w\-\s]))([\w\-]{11})[a-z0-9;:@?&%=+\/\$_.-]*/$1/i;
  $self->send_params->{vimeolink}
    =~ s/(?:http|https|)(?::\/\/|)(?:player\.|)?vimeo\.com(?:\/video)?\/(\d+)/$1/i;
