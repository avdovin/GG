...
{ # Создание нового класса. Отсюда начинается класс
  my ($class, %cfg)
    = @_;    #Mod - список модулей для загрузки
  my $self = bless {}, $class;
...
#'root'         	=> 	'../../httpdocs/userfiles',           # path to root directory
    ...
    'volume_id' => 'gg_',
    'copyOverwrite' => 1
    , # Разрешена или нет перезапись файлов с одинаковыми именами на текущем томе
    ...
    'mimeDetect' => 'auto'
    , # files mimetypes detection method (finfo, mime_content_type, linux (file -ib), bsd (file -Ib), internal (by extensions))
    'uploadAllow' => [],    # mimetypes which allowed to upload
    'uploadDeny'  => [],    # mimetypes which not allowed to upload
    'uploadOrder' =>
      'deny,allow',    # order to proccess uploadAllow and uploadAllow options
    ...
    '@WeekDays' =>
      'Воскресенье,Понедельник,Вторник,Среда,Четвер,Пятница,Суббота',
...
sub __create_tmp {
  ...
  if ($mime && $mime =~ /image/) {
    ...
    unless (-d $dir . $self->{CONF}->{tmbDir} . $DIRECTORY_SEPARATOR) {
      mkdir($dir . $self->{CONF}->{tmbDir} . $DIRECTORY_SEPARATOR,
        $self->{CONF}->{'dirMode'});
    }

    eval {
      File::Copy::Recursive::fcopy($dir . $filename,
        $dir . $self->{CONF}->{tmbDir} . $DIRECTORY_SEPARATOR . $filename)
        or die $!;
    };
...
#next if (substr($subdir, -length($self->{CONF}->{tmbDir})) eq  $self->{CONF}->{tmbDir});
    ...
    @$dirs = (@$dirs, @{$self->__tree("$path/$subdir", $depth - 1)})
      if $depth > 1;
  }
  return $dirs;
...
sub _options {
  ...
  %{$self->{RES}->{'options'}} = (
    ...
    'url'  => $self->{CONF}->{URL}
      . $DIRECTORY_SEPARATOR
    ,    # $self->_path2baseUrl($path).$DIRECTORY_SEPARATOR,
    'tmbUrl' => $self->_path2baseUrl($path)
      . $DIRECTORY_SEPARATOR
      . $self->{CONF}->{tmbDir}
      . $DIRECTORY_SEPARATOR,
...
#'tmbUrl'		=> $self->{CONF}->{tmbDir} ? $self->{CONF}->{URL}.$DIRECTORY_SEPARATOR.$self->{CONF}->{tmbDir}.$DIRECTORY_SEPARATOR : 'False',
    ...
    'separator'     => $DIRECTORY_SEPARATOR,
    'archivers' =>
      {'create' => [qw(application/zip)], 'extract' => [qw(application/zip)],},
  );
}
...
#my @files = (@{ $self->{RES}->{'files'} }, @{ $self->__tree($path, $self->{REQUEST}->{'tree'}) } );
    my @files = (
      @{$self->{RES}->{'files'}},
      @{$self->__tree(undef, $self->{REQUEST}->{'tree'})}
    );
...
sub _info {
  ...
  %info = (
    ...
    'name' => _basename($path)
    ,    # Сделать замену двойных кавычек
  ...
  if ($info{'mime'} ne 'directory') {
    ...
    if ($info{'mime'} =~ /image/) {
      ...
      if (
        -f $dir . $self->{CONF}->{tmbDir} . $DIRECTORY_SEPARATOR . $info{name})
      {
...
# (Number) Only for directories. Marks if directory has child directories inside it. 0 (or not set) - no, 1 - yes. Do not need to calculate amount.
  ...
  if ($self->{'cache_on'}) {
    ...
    $self->{'app'}->dbi->dbh->do(
      "REPLACE INTO `sys_filemanager_cache`(`hash`,`info`, `updated`) VALUES (?, ?, NOW()) ",
      undef, $hash, $encodeData
    );
  }
...
sub _rename {
  ...
  if (!$self->{REQUEST}->{'target'}) {
    return $self->{RES}->{'error'}
      .= 'Не указаны объект для переименования';
  }
  ...
  eval {
    rename(
      $dir . $DIRECTORY_SEPARATOR . $old_name,
  ...
  push @{$self->{RES}->{'added'}},
    {$self->_info($dir . $DIRECTORY_SEPARATOR . $new_name)};
...
sub _extract {
  ...
  foreach my $element ($zip->members) {
    ...
    $element->extractToFileNamed($dir
        . $DIRECTORY_SEPARATOR
        . $foldername
        . $DIRECTORY_SEPARATOR
        . $filename);
  ...
  $self->{RES}->{'added'}
    = [{$self->_info($dir . $DIRECTORY_SEPARATOR . $foldername)}];
}
...
sub _archive {
  ...
  if (!$self->{REQUEST}->{'targets[]'}) {
    return $self->{RES}->{'error'}
      .= 'Не указаны файлы/папки для архивации';
  }

  if ($self->{REQUEST}->{'type'} ne 'application/zip') {
    return $self->{RES}->{'error'}
      .= 'Не поддерживаемый тип архива';
  }
  ...
  my $uniquename = $self->{app}
    ->file_check_free_name($dir . $DIRECTORY_SEPARATOR . 'Archive1.zip');

  unless (
    $zip->writeToFileNamed($dir . $DIRECTORY_SEPARATOR . $uniquename) == AZ_OK)
  {
    return $self->{RES}->{'error'}
...
sub _resize {
  ...
  if (!$self->{REQUEST}->{'target'}) {
    return $self->{RES}->{'error'}
      .= 'Не указаны файлы для создания дубликатов';
  }
  ...
  if ($mode eq 'resize') {
    ...
    my ($w, $h, $x, $y) = (
      ...
      $self->{REQUEST}->{'x'},     $self->{REQUEST}->{'y'}
    );

    $self->{app}->image_crop_raw(
      file   => $path,
      width  => $w,
      height => $h,
      x      => $x,
    ...
    eval {
      ...
      $image->Rotate(degrees => $degree, background => 'red',);
      $x = $image->Write($path);
      undef $image;
    };
    if ($@) {
    ...
    eval {
      ...
      if (-d $dir . $DIRECTORY_SEPARATOR . $name) {
        ...
        File::Copy::Recursive::dircopy(
          ...
          $dir . $DIRECTORY_SEPARATOR . $copyname
          )
          or die $!
        ...
        my $ext = ($name =~ m/([^.]+)$/)[0];
        $name =~ s{\.$ext$}{}e;
        $copyname = $self->{app}->file_check_free_name(
          $dir . $DIRECTORY_SEPARATOR . $name . ' copy 1.' . $ext);
        $name = $save_name;
...
# Прекращаем выполнение если произошла ошибка
    if ($@) {
      $self->{RES}->{'error'}
        = "Ошибка при создание дубликатов: $@";
      last;
    }
    else {
...
sub _paste {
  ...
  if (!$self->{REQUEST}->{'targets[]'}) {
    return $self->{RES}->{'error'}
      .= 'Не указаны файлы для копирования/перемещения';
  }
  ...
  unless ($src) {
    return $self->{RES}->{'error'}
      .= 'Не задана папка источник';
  }
  unless ($dst) {
    return $self->{RES}->{'error'}
      .= 'Не задана папка приемник';
  }
...
#next if( (-d $src.$name && -d $dst.$name) or (-f $src.$name && -f $dst.$name) );
    ...
    eval {
      ...
      if (-d $src . $DIRECTORY_SEPARATOR . $name) {
        File::Copy::Recursive::dircopy(
          $src . $DIRECTORY_SEPARATOR . $name,
          $dst . $DIRECTORY_SEPARATOR . $name
          )
          or die $!
...
# Прекращаем выполнение если произошла ошибка
    if ($@) {
      $self->{RES}->{'error'}
        = "Ошибка при копирование/перемещении: $@";
      last;
    }
    else {
...
sub _mkfile {
  ...
  eval {
    open(FILE, ">${dstpath}${DIRECTORY_SEPARATOR}$name")
      or die("can't open ${dstpath}${DIRECTORY_SEPARATOR}$name: $!");
    flock(FILE, 2);
    binmode(FILE);
  ...
  $self->{RES}->{'added'}
    = [{$self->_info($dstpath . $DIRECTORY_SEPARATOR . $name)}];
}
...
sub _upload {
  ...
  return $self->{RES}->{'error'}
    ...
    if $app->req->is_limit_exceeded;

  my $filemanager_max_image_size
    = $app->get_var('filemanager_max_image_size') || 0;
  ...
  for my $file (@{$app->req->uploads('upload[]')}) {
    ...
    chmod(
      $dstpath . $DIRECTORY_SEPARATOR . $filename,
      $self->{CONF}->{'fileMode'}
    );

    if ($filemanager_max_image_size) {
      $app->resize_pict(
        fsize => $filemanager_max_image_size,
        file  => $dstpath . $DIRECTORY_SEPARATOR . $filename
      );
    }

    # clear cache
    my $path_hash
      = $self->_hash_api2_encode($dstpath . $DIRECTORY_SEPARATOR . $filename);
    if ($cache->get($path_hash)) {
      $cache->set($path_hash => undef);
      $self->{'app'}
        ->dbi->dbh->do("DELETE FROM sys_filemanager_cache WHERE hash=?",
        undef, $path_hash);
    }

    push @{$self->{RES}->{'added'}},
      {$self->_info($dstpath . $DIRECTORY_SEPARATOR . $filename)};
  }
}
...
sub _findDir {
  ...
  foreach my $subdir (grep { -d "$path/$_" } @content) {
    ...
    if ($self->_hash_api2_encode($p) eq $hash
      || ($p = _findDir($self, $hash, $p)) ne 'false')
    {
      last;
    }
  }
...
sub _abs_path {
  ...
  return $path eq $DIRECTORY_SEPARATOR
    ? $self->{CONF}{'root'}
    : $self->{CONF}{'root'} . $path;
}
...
sub _rel_path {
  ...
  return $path eq $self->{CONF}{'root'}
    ? ''
    : $DIRECTORY_SEPARATOR . substr($path, length($self->{CONF}{'root'}) + 1);
}
...
sub _hash_api2_decode {
  ...
  if (
    substr($hash, 0, length($self->{CONF}->{volume_id})) eq
    $self->{CONF}->{volume_id})
  {
