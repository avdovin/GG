...
sub restart_fcgi {
  ...
  if ($self->config->{bash_restart_cmd}) {
    ...
    $bash_command
      = "killall -u " . $self->config->{ftp_username} . " dispatch.fcgi";
  }
  else {
  ...
  system($bash_command) == 0 or die "system failed: $?";
...
sub mainpage {
  ...
  $self->sysuser->save_settings(lang => 'ru')
    unless $self->sysuser->settings->{lang};
  ...
  for my $row (
    $self->dbi->query("SELECT * FROM `sys_program` WHERE `$user_lang`='1'")
    ->hashes)
  {

    my $button
      = $app->button(name => $$row{key_razdel}, controller => 'global');
    ...
    if ($self->sysuser->sys
      or exists($self->sysuser->access->{modul}->{$$row{ID}}))
    {

      if ($row->{key_razdel} eq 'texts') {
        my $access_where
          = $self->def_access_where(base => 'lst_texts', show_empty => 0);

        $self->getHashSQL(
          select => '*',
          from   => 'lst_texts',
          where  => "`"
            . $self->sysuser->settings->{'lang'}
            . "`='1' $access_where",
          sys   => 1,
          stash => 'texts'
        );
        ...
        foreach my $r (@$razdel) {
          ...
          my $script
            = "ld_content('$$settings{replaceme}','$$settings{program}&$$button{params_string}&razdel=$$r{ID}')";
  ...
  foreach (sort keys %$panelsList) {
    $body
      .= qq{<tr><td><br><h2>$$panelsList{$_}:</h2><br>$main_menu{$_}</td></tr>}
      if $main_menu{$_};
  }
...
sub user_block {
  ...
  if (
    $self->getArraySQL(
      ...
      from   => "sys_users",
      where =>
        "`login` = '$login' and ((unix_timestamp(NOW()) - unix_timestamp(`vdate`) > 60*60*3) or (`vdate` = '0000-00-00 00:00:00'))",
      sys => 1,
    )
    )
  {
...
sub menu_top {
  ...
  if (
    $ENV{'HTTP_USER_AGENT'}
    && ( $ENV{'HTTP_USER_AGENT'} =~ /MSIE 8.0/
      or $ENV{'HTTP_USER_AGENT'} =~ /MSIE 7.0/)
    )
  {
    $is_msie = 1;
  }
  ...
  my $rows
    = $self->dbi->query(
    "SELECT `ID`,`name`,`menu_btn_key`,`key_razdel` FROM `sys_program` WHERE `$user_lang`='1' AND `prgroup` IN (1, 2, 3) ORDER BY `prgroup`, `name`"
    )->hashes
    || [];

  push @$rows,
    {ID => 999999, key_razdel => 'settings', name => 'Настройки',};
  ...
  for my $row (@$rows) {
    next
      if (!$self->sysuser->sys
      and !exists($self->sysuser->access->{modul}->{$$row{ID}}));
...
#my $icon = $app->buttons->{ $$row{'key_razdel'} }->{'settings'}->{'imageiconmenu'} || '';
      ...
      push @$items,
        {
        type  => 'eval',
        value => "menuTop.addItem("
          . $moduls->{$row->{ID}}
          . ",'$$row{name}','','',false,'$$row{name}','');"
        };
      ...
      if (my $button_key = $$row{'key_razdel'} . '_topmenu') {
          ...
          )
        {
          ...
          next
            if (!$$button{settings}{$button_key}
            or (!$access_buttons->{$b}->{r} and !$user_sys));

          if ($button->{settings}->{'do'}
            && substr($button->{settings}->{'do'}, 0, 14) eq 'list_container')
          {
            $button->{settings}->{'do'}
              = 'enter' . substr($button->{settings}->{'do'}, 14);
          }
...
# Скрываем иконки т.к. IE 7/8 не поддерживают css background-size
          ...
          if ($b eq 'texts') {
            ...
            foreach my $r (@$razdel) {
              ...
              if ( $user_sys
                or $self->sysuser->access->{table}->{$txtTable}->{w})
              {
                my $script
                  = "ld_content('$$settings{replaceme}','$$settings{program}&$$button{params_string}&razdel=$$r{ID}')";
                push @$items,
            ...
            # Добавление записи в тестовый раздел
                ...
                if (
                  $buttonByController->{add_link}
                  && (
                    $user_sys
                    or ( $self->sysuser->access->{button}->{add_link}
                      && $self->sysuser->access->{button}->{add_link}->{r})
                  )
                  )
                {
                  my $buttonAdd = $$buttonByController{'add_link'};
                  $buttonAdd->def_params_button({controller => $b,});
                  $buttonAdd->def_script_button();
                  my $settingsAddBtn = $buttonAdd->{settings};
                  my $scriptAdd
                    = "ld_content('$$settings{replaceme}','$$settingsAddBtn{program}&$$button{params_string}&razdel=$$r{ID}&list_table=$txtTable')";
                  ...
                  push @$items,
                    ...
                    value => "menuTop.addItem("
                      ...
                      . $sch
                      . $moduls->{$row->{ID}}
                      . ",'Добавить запись','/admin/img/icons/menu/icon_add.png','',"
                      . $sch
                      . $moduls->{$row->{ID}}
                      . ",'',\"$scriptAdd\")",
                    };
            ...
            push @$items,
              ...
              value => "menuTop.addItem("
                ...
                . ",\"$$button{name}\",\"$icon\",\"\","
                . $moduls->{$row->{ID}}
                . ", \"$$button{name}\" ,\"$$button{settings}{script}\");"
              };
  ...
  @$items = (
    @$items,
    (
      {type => 'eval', value => "menuTop.init();"},
      {type => 'eval', value => "menuTopBar.addMenuItems(menuTop);"},
      {type => 'eval', value => "menuTopBar.setTarget('innerDiv');"},
      {type => 'eval', value => "menuTopBar.init();"},
    )
  );

  $self->render(json => {content => 'OK', items => $items,});
}
