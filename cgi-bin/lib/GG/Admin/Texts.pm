...
#my $access_where = $self->def_access_where( base => $self->stash->{txt_razdel}, show_empty => 0);
  ...
  $self->app->lkeys->{texts}->{razdel}->{settings}->{where}
    = " AND `" . $self->sysuser->settings->{'lang'} . "`='1' ";

  $self->stash->{list_table}
    = 'texts_'
    . $self->stash->{key_razdel} . '_'
...
sub changeRazdel {
  ...
  $self->sysuser->save_ses_settings(
    texts_razdel => $self->send_params->{razdel});
  $self->sysuser->save_ses_settings(
...
sub lists_select {
  ...
  $menu
    .= "document.getElementById('ok_' + out).innerHTML = \"<span style='background-color:lightgreen;width:45px;padding:3px'>найдено: $sch</span>\";\n";
  $menu .= $tmp;
...
sub tree_block {
  ...
  my $existFields = {
    ...
    $key_razdel => $self->dbi->exists_keys(from => $table, lkey => $key_razdel)
    ? 1
    : 0,
  ...
  if ($existFields->{tdate}) {
    ...
    if ($existFields->{'dir'}) {
      $stash->{count} = $self->dbi->getCountCol(
        from  => $table,
        where => "`dir`='0' AND `$key_razdel`='" . $stash->{pid} . "'"
      );
      $stash->{count_dir} = $self->dbi->getCountCol(
        from  => $table,
        where => "`dir`='1' AND `$key_razdel`='" . $stash->{pid} . "'"
      );
    }
  }
  ...
  if (!$stash->{count_dir} and $existFields->{tdate}) {
    ...
    if (
      $self->sysuser->access->{table}->{$table}->{r}
      && $self->getHashSQL(
        select => "`ID`,`name`"
          . ($existFields->{'dir'} ? ',`dir`' : '')
          . " $select",
        from  => $table,
        where => "`tdate`!='0000-00-00' "
          . (
          $existFields->{$key_razdel}
          ? "AND `$key_razdel`='" . $stash->{pid} . "'"
          : ""
          )
          . " $where ORDER BY `ID`",
        stash => "items"
      ...
      )
    {
      ...
      foreach my $i (0 .. $#$items) {
        ...
        {
          $param_default
            = "&year=" . $send_params->{year} . "&month=$$item{name}";
          $items->[$i]->{params} = {
            tdate => sprintf("%04d-%02d-%02d",
              $send_params->{year},, $$item{name}, '00'),
            razdel => $self->stash->{razdel},
          };
          ...
          and !$send_params->{day})
        {
          ...
          $items->[$i]->{params} = {
            tdate => sprintf("%04d-%02d-%02d",
              $send_params->{year},, $send_params->{month}, $$item{name}),
            razdel => $self->stash->{razdel},
          };
      ...
      )
    {
      ...
      foreach my $i (0 .. $#$items) {
        ...
        if ($item->{dir}) {
          ...
          my $count = $self->dbi->getCountCol(
            from  => $table,
            where => "`$key_razdel`='" . $item->{ID} . "'"
          );
...
# проверяем что данный модуль уже не выбран на других страницах сайта
  if (my $url_for = $self->send_params->{url_for}) {
    if (
      my $item = $self->dbi->query(
            "SELECT `ID`, `name` FROM `"
          . $self->stash->{list_table}
          . "` WHERE `url_for`='$url_for' "
          . ($self->stash->{index} ? ' AND `ID`!=' . $self->stash->{index} : '')
      )->hash
      )
    {
      $self->admin_msg_errors(
        "Данный модуль уже используется на странице - $item->{name} # $item->{ID}"
      );
      $self->edit;
    }
  }
  ...
  if ($self->save_info(%params, table => $self->stash->{list_table})) {
    ...
    if ($params{continue}) {
      ...
      && $self->stash->{group} >= $#{$self->app->program->{groupname}} + 1)
    {
      return $self->info;
    }
    $self->stash->{group}++;
...
sub delete_list_items {
  my $self = shift;
  my %params = (table => $self->stash->{list_table}, @_,);
  ...
  foreach my $id (split(/,/, $list_items)) {
    ...
    if (my $item = $self->getArraySQL(from => $table, where => $id, sys => 1)) {
      if ( $item
        && $item->{dir}
        && $self->dbi->exists_keys(from => $table, lkey => $dir_field))
      {
        if (
          $self->getArraySQL(
            from  => $table,
...
# }
        ...
        $self->save_logs(
          name => 'Удаление записи из таблицы ' . $table,
