...
sub lists_select {
  ...
  if ($lfield eq 'groups') {
    ...
    for my $item (
      $self->dbi->query(
        "SELECT `value`,`name`,`valuename` FROM `lst_catalog_dimensions` WHERE $where ORDER BY `name`"
      )->hashes
      )
    {
      ...
      $list_out
        .= "lstobj[out].options[lstobj[out].options.length] = new Option($name, $value);\n"
        if $name;
      $sch++;
    }
    ...
    for my $item (
      $self->dbi->query(
        "SELECT distinct(code), `code` FROM `lst_catalog_dimensions` WHERE $where ORDER BY `code` "
      )->hashes
      )
    {
