...
sub register {
	...
	$app->helper(
		export_config => sub {
			...
			$self->get_keys( type => ['lkey', 'list'], keys_table => $self->app->program->{keys_table});
			...
			my %types = (table => 1, text => 1, html => 1, password => 1, code => 1, file => 1, filename => 1, pict => 1);
			...
			if ($group == 1) {
				...
				return $self->file_download( abs_path => $self->file_tmpdir.$self->send_params->{filename});
			} 
	...
	$app->helper(
		export_excel_get_items => sub {
			...
			$self->app->sysuser->settings->{$self->stash->{replaceme}."_defcol"} = $listfieild_headers;
			
			$self->def_listfield( table => $table, lkey => $self->stash->{replaceme});
			$self->filter_take(lkey => $self->stash->{replaceme});
			
			my $where_filtered = $self->set_filter( lkey => $self->stash->{replaceme}, table => $table);
			
			my $items = [];
			if ($self->sysuser->access->{table}->{$table}->{r} || $self->sysuser->sys) {
				my $listfields = $self->stash->{listfields};
				my $listfield = $self->stash->{listfields} ||= [];
				$items = $self->getHashSQL(	select => join(",", map{"`$table`.`$_`"}@{$listfield}),
											from   => $self->stash->{table_from},
											where  => "1 $params{where} $where_filtered ORDER BY `$table`.`ID` ".$self->stash->{asc}." LIMIT ".$self->stash->{page}.",".$self->stash->{pcol}." ",
											sys    => 1
				) || [];
			}
...
#			return $params{return} || $self -> defBlock(shablon => "eexport_win_close", from => "shablon_reload");			
	...
	$app->helper(
		export_excel_build => sub {
			...
			my $filename = $params{filename} || $self->transliteration( sprintf("%s_%04d_%02d_%02d.xls", $self->stash->{controller_name}, (localtime)[5]+1900, (localtime)[4]+1, (localtime)[3]) );
			...
			my $eexcel_key = $self->send_params->{eexcel_key};
			...
			my $length = $eexcel_key ? scalar(@{$listfield_headers}) - 1 : scalar(@{$listfield_headers});
			...
			foreach my $item (@$items){
				...
				my $format = $self->stash->{_rowcount} & 1 ? $formats->{'blank'} : $formats->{'blank_xRow'};
				foreach my $key (@{$listfield_headers}) {
					next if($eexcel_key && $key eq 'ID');
					...
					$self->export_excel_build_write_cell($oWs, (string => $value, rowN => 0, colN => 1, format => $format));

				}
				$self->export_excel_build_write_cell($oWs, (string => '', rowN => 1, colN => -1, format => $format));
			}

			my $url = $self->url_for('admin_routes', controller => 'main', action => 'body')."?do=excel&group=3&filename=$filename&key_program=".$self->param('key_program')."&list_table=".$self->param('list_table');
			$self->render_json({
				content	=> "OK",
				items	=> [
					{
	...
	$app->helper(
		export_excel_build_write_cell => sub {
			...
			$oWs->write($self->stash->{_rowcount}, $self->stash->{_colcount}, $string, $params{format});		
			
			$self->stash->{_rowcount}++ if($params{rowN} == 1); # двигаемся к следующим ячейкам
			$self->stash->{_colcount}++ if($params{colN} == 1);
			$self->stash->{_colcount} = 0 if($params{colN} < 0); # возвращаемся вначало
		}
	);
}
...
sub _eAddFormats {
	...
	$formats->{'blank'} = $oBook -> add_format(color => 'black', num_format => '@', text_wrap => 1);
	$formats->{'blank_xRow'} = $oBook->add_format(bg_color	=> 10, color => 'black', num_format => '@', text_wrap => 1);
	$formats->{'title'} = $oBook -> add_format(bold => 1, size => 10, bg_color => 'silver', color => 'black');
