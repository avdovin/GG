...
sub tree_block {
  ...
  my $items
    = $self->getHashSQL(from => $self->param('index'), where => "`ID`>0");

  my $table = $self->stash->{list_table};
  foreach my $i (0 .. $#$items) {
    $items->[$i]->{name}
      = "<b>" . $items->[$i]->{lkey} . "</b> " . $items->[$i]->{name};
...
sub list_container {
  ...
  if (
    my $controller = $self->dbi->query(
      "SELECT `name` FROM `sys_program` WHERE `keys_table`='"
...
#"script"		=> "openPage('center','keys${t}','/admin/keys/body?do=list_container&list_table=$t','Ключи: $t','$t')"
        "script" =>
          "ld_content('replaceme','/admin/keys/body?do=list_container&list_table=$t')"
      );
