...
sub inventory {
  ...
  copy(
    $ENV{DOCUMENT_ROOT} . '/import/inventory.xml',
    $ENV{DOCUMENT_ROOT} . '/import/inventory_tmp.xml',
    8 * 1024
  );
  ...
  my $ref
    = XMLin($ENV{DOCUMENT_ROOT} . '/import/inventory_tmp.xml', ForceArray => 1);
  ...
  my $sth
    = $dbh->prepare(
    "UPDATE `data_catalog_items` SET `quantity`=? WHERE `sync`=? AND `variantcode`=?"
    );
  ...
  copy(
    $ENV{DOCUMENT_ROOT} . '/import/inventory_tmp.xml',
    $ENV{DOCUMENT_ROOT} . '/import/backup/inventory.xml',
    8 * 1024
  );
...
sub imgs {
  ...
  copy(
    $ENV{DOCUMENT_ROOT} . '/import/images.zip',
    $ENV{DOCUMENT_ROOT} . '/import/images_tmp.zip',
    8 * 1024
  );
  ...
  my $imgs = $self->file_extract_zip(
    path => $ENV{DOCUMENT_ROOT} . '/import/images_tmp.zip');
  my $sth = $dbh->prepare(
    "UPDATE `data_catalog_items` SET `images`=1 WHERE `sync`=?");

  my $sth_check_item = $dbh->prepare(
    "SELECT `sync` FROM `data_catalog_items` WHERE `sync`=? LIMIT 0,1");
...
# Пропускаем картинку если такого товара нет в базе
    ...
    foreach my $d (split(/,/, $MINI)) {
      if ($d =~ /([\d]+)x([\d]+)~([\w]+)/) {
        ...
        my ($path, $pict, $ext) = $self->file_save_from_tmp(
          ...
          to       => $imgFolder . $img->{filename},
          prefix   => $w . "x" . $h
        );

        $self->resize_pict(file => $path, width => $w, height => $h, %type);
      }
      elsif ($d =~ /([\d]+)x([\d]+)/) {
        my ($w, $h) = ($1, $2);
        ...
        $self->resize_pict(file => $path, width => $w, height => $h);
        ...
        $self->resize_pict(file => $path, fsize => $d,);
      }
  ...
  copy(
    $ENV{DOCUMENT_ROOT} . '/import/images_tmp.zip',
    $ENV{DOCUMENT_ROOT} . '/import/backup/images.zip',
    8 * 1024
  );
