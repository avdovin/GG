...
my $Access = {modul => {}, table => {}, lkey => {}, button => {}, menu => {}};
...
sub save_ses_settings {
  ...
  $self->{dbi}->{dbh}->do(
    "UPDATE `sys_users_session` SET `data`=? WHERE `id_user`=? AND `cck`=?",
    undef, $encodeData,
    $$self{userinfo}{ID},
    $$self{userinfo}{cck}
  );
...
sub save_user_settings {
  ...
  foreach (keys %{$$self{user_settings}}) {
    push(@settings, "$_=$$self{user_settings}{$_}")
      if ($$self{user_settings}{$_}
      and length($$self{user_settings}{$_}) > 0
      and !exists($settings{$_}));
    $settings{$_} = 1;
  }
  my $set_save = join("\n", @settings);

  $self->{dbi}->{dbh}->do("UPDATE `sys_users` SET `settings`=? WHERE `ID`=?",
    undef, $set_save, $$self{userinfo}{ID});
}
...
sub check_modul_access {
  ...
  push(@id_group_user, "`id_group_user`='$_'")
    foreach (split(/=/, $user->{id_group_user}));
  ...
  if ($user->{sys}) {
    my %modul_access = ();
    for my $row (
      $self->{dbi}->query("SELECT `ID` FROM `sys_program` WHERE 1")->hashes)
    {
    ...
    if ($id_group_user) {
      ...
      my $sql
        = "SELECT * FROM `sys_access` WHERE ($id_group_user) AND `r`='1' AND `objecttype`='modul'";

      $self->check_rules($sql, 0);

      # Исключение правил, где доступ запрещен
      $sql
        = "SELECT * FROM `sys_access` WHERE ($id_group_user) AND `r`='0' and `objecttype`='modul'";
...
# Проверка правил, где доступ разрешен
    my $sql
      = "SELECT * FROM `sys_access` WHERE `users`='$$user{ID}' AND `r`='1' and `objecttype`='modul'";
    $self->check_rules($sql, 0);
    #
    #	# Исключение правил, где доступ запрещен
    $sql
      = "SELECT * FROM `sys_access` WHERE `users`='$$user{ID}' and `r`='0' and `objecttype`='modul'";
    $self->check_rules($sql, 1);

    if ($params{access})
    { # Проверка разрешен ли доступ к конкретному модулю
      $sql
        = "SELECT `ID` AS `index_modul` FROM `sys_program` WHERE `key_razdel`='$params{access}'";
...
sub settings {
  ...
  my %settings = ();
  $settings{$_} = $self->user_settings->{$_}
    foreach (keys %{$self->user_settings});
  $settings{$_} = $self->ses_settings->{$_}
    foreach (keys %{$self->ses_settings});
...
sub store_access {
  ...
  $self->{dbi}->{dbh}->do("UPDATE `sys_users` SET `access`=? WHERE `ID`=?",
    undef, $encodeData, $self->userinfo->{ID});
  return 1;
}
...
sub getAccess {
  ...
  my (@id_group_user);
  push(@id_group_user, "`sys_access`.`id_group_user`='$_'")
    foreach (split(/=/, $user->{id_group_user}));
  ...
  if ($id_group_user) {
    $params{where}
      = "($id_group_user OR `users`='$$user{ID}') AND `objecttype`='table'";
  }
  else {
  ...
  for my $row ($self->{dbi}
    ->query("SELECT * FROM `sys_access` WHERE `ID`>0 AND $params{where}")
    ->hashes)
  {
...
# Проставляем доступ всем пользователям на таблицы sys_*
  unless ($self->sys) {
    ...
    $self->sys(1);
    $self->set_access("table", {objectname => join('=', @$sysTables)});
  ...
  if ($id_group_user) {
    $params{where}
      = "($id_group_user OR `sys_access`.`users`='$$user{ID}') AND `sys_access`.`objecttype`='lkey'";
  }
  else {
    $params{where}
      = "(`sys_access`.`users`='$$user{ID}') AND `sys_access`.`objecttype`='lkey'";
  }
    ...
    for my $row (
      $self->{dbi}->query(
        qq/
			...
			WHERE `sys_access`.`ID`>0 AND $params{where} AND `sys_program`.`key_razdel`='$params{modul}'/
      )->hashes
      )
    {
  ...
  if ($id_group_user) {
    $params{where}
      = "($id_group_user OR `sys_access`.`users`='$$user{ID}') AND `sys_access`.`objecttype`='button'";
  }
  else {
    $params{where}
      = "(`sys_access`.`users`='$$user{ID}') AND `sys_access`.`objecttype`='button'";
  }
  if ($params{modul}) {
    for my $row (
      $self->{dbi}->query(
  ...
  for my $row (
    $self->{dbi}->query(
      qq/
		SELECT `sys_access`.*
		FROM `sys_access` LEFT JOIN `sys_program` ON `sys_access`.`modul`=`sys_program`.`ID`
		WHERE `sys_access`.`ID`>0 AND $params{where} AND `sys_program`.`key_razdel`='global'/
    )->hashes
    )
  {
  ...
  if ($id_group_user) {
    $params{where}
      = "($id_group_user OR `sys_access`.`users`='$$user{ID}') AND `sys_access`.`objecttype`='menu'";
  }
  else {
    $params{where}
      = "(`sys_access`.`users`='$$user{ID}') AND `sys_access`.`objecttype`='menu'";
  }
  for my $row (
    $self->{dbi}->query(
      qq/
		SELECT `sys_access`.*
		FROM `sys_access` LEFT JOIN `sys_program` ON `sys_access`.`modul`=`sys_program`.`ID`
		WHERE `sys_access`.`ID`>0 AND $params{where}/
    )->hashes
    )
  {
...
sub set_access {
  ...
  if (@_ == 2) {
    ...
    if ((defined $a and defined $row_hashref->{cck})
      && $self->def_cck_access($a) eq $row_hashref->{cck})
    {
      $rule_ok = 1;
    }
    ...
    foreach my $obj (split(/=/, $row_hashref->{objectname})) {
      foreach my $k (@keys) {
        if ($rule_ok || $user_sys) {
          $self->access->{$rule}->{$obj}->{$k}
            = $user_sys ? 1 : $row_hashref->{$k};
        }
        else {
          $self->access->{$rule}->{$obj}->{$k} = 0;
        }
      }
      $self->access->{$rule}->{$obj}->{w} = 0
        unless $self->access->{$rule}->{$obj}->{r};
    }
  }
  else {
