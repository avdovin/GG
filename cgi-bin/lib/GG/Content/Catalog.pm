...
sub brands{
  ...
  my $items = $self->dbi->query("SELECT * FROM `data_catalog_brands` WHERE `active`=1 ORDER BY rating,name")->hashes;
...
sub order_checkout{
  ...
  my $fields = {
	...
	if(keys %$errors){
		$json->{errors} = $errors;
    $json->{errors}->{'misc'} = 'ВЫ НЕ ЗАПОЛНИЛИ ОБЯЗАТЕЛЬНЫЕ ПОЛЯ';
	...
	my $items = $self->dbi->query("
        ...
        LEFT JOIN lst_catalog_colors
          ON data_catalog_items.color_id=lst_catalog_colors.ID
				WHERE data_catalog_items.ID IN (".join(',', keys %$basket).") AND data_catalog_items.active=1 ")->hashes || [];
...
sub update_basket_items{
	my $self = shift;
	my %params = (
		delFlag => $self->param('delFlag') ? 1 : 0,
		@_
	);
  ...
  warn $delFlag;
      ...
      if($self->userdata->{basket}->{ $item->{id} }){
        # update
        $self->userdata->{basket}->{ $item->{id} }->{'count'} = $item->{count} if $item->{count};
        $self->userdata->{basket}->{ $item->{id} }->{'size'} = $item->{count} if $item->{size};
        $self->userdata->{basket}->{ $item->{id} }->{'footsize'} = $item->{count} if $item->{footsize};
      }
      else {
  ...
  $json->{price_formated} = sprintf("%s %s", $self->numberformat($json->{price}), 'руб.' );
  $json->{count_formated} = sprintf("%s %s", $self->numberformat($json->{count}),  $self->pluralize($json->{count}, [qw(товар товары товаров)] )  );
...
sub iteminfo{
  ...
  /)->hash;
	...
	if($item->{recommend}){
		...
		$recommendItems = $self->dbi->query(qq/
  ...
  my $category = {};
  if($item->{category_id}){
    $category = $self->dbi->query("SELECT ID,name,alias FROM `data_catalog_categorys` WHERE ID=$$item{category_id} ")->hash;
  }
  my $subcategory = {};
  if($item->{subcategory_id}){
    $subcategory = $self->dbi->query("SELECT ID,name,alias FROM `data_catalog_categorys` WHERE ID=$$item{subcategory_id} ")->hash;
  }
...
sub list_items {
	my $self = shift;
	my %params = (
		limit 	  => $self->get_var(name => 'catalog_items_limit', controller => 'catalog', raw => 1),
...
#		});
  ...
  my $total_string = sprintf("%s %s", $self->numberformat($self->stash->{'count'}), $self->pluralize($self->stash->{'count'}, [qw(товар товара товаров)])  );
...
sub _build_where{
  ...
  if($sendParams->{category_id}){
    $where .= " AND data_catalog_items.category_id='".$sendParams->{category_id}."' ";
  }
  if($sendParams->{subcategory_id}){
    $where .= " AND data_catalog_items.subcategory_id='".$sendParams->{subcategory_id}."' ";
  }
  ...
  if(my $gender = $sendParams->{gender}){
    ...
    $where .= " AND data_catalog_items.gender IN (".join(',', @$gender_arr).") " if scalar @$gender_arr;
  }
  ...
  if(my $sizes = $sendParams->{'sizes'}){
    ...
    $where .= " AND data_catalog_items.ID IN (SELECT item_id FROM dtbl_catalog_item_sizes WHERE size IN (".join(',', @$sizes_arr).") ) " if scalar @$sizes_arr; 
  }
  if(my $colors = $sendParams->{'colors'}){
    ...
    $where .= " AND data_catalog_items.color_id IN (".join(',', @$color_arr).") " if scalar @$color_arr; 
  }
