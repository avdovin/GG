...
sub texts_list {
  ...
  my $table
    = "texts_"
    . $key_razdel . "_"
    . ($params{lang_tables} ? $self->lang : 'ru');
  my $template = $key_razdel . '_list';

  if ($self->stash->{alias}) {

    if (
      my $mainPage = $self->dbi->query(
        "SELECT `name`,`title`,`keywords`,`description` FROM `texts_main_ru` WHERE `alias`='"
          . $self->stash->{alias}
          . "' AND `viewtext`='1'"
      )->hash
      )
  ...
  if ($self->stash->{level}) {
    ...
    my $last_item
      = $self->dbi->query(
      "SELECT `dir` FROM `$table` WHERE `viewtext`='1' AND `alias`='$level'")
      ->hash;
...
# Условия выборки:
  my $where = " `viewtext`='1' ";
  $where .= " AND `dir`='0'"
    if $self->dbi->exists_keys(from => $table, lkey => 'dir');
  $where .= " AND YEAR($params{order_field})='" . $self->param('year') . "' "
    if $self->param('year');
  $where .= " AND MONTH($params{order_field})='" . $self->param('month') . "' "
    if $self->param('month');
  $where .= " $params{where} "
    if $params{where}
  ...
  if ($self->stash->{group_alias}) {
    return $self->render_not_found
      unless my $parentDir = my $mainPage
      = $self->dbi->query("SELECT * FROM `$table` WHERE `dir`='1' AND `alias`='"
        . $self->stash->{group_alias}
        . "' AND `viewtext`='1'")->hash;

    $where .= " AND `$table`='$$parentDir{ID}' ";
    $self->metaHeader(
      title => $parentDir->{title} || $parentDir->{name},
      keywords    => $parentDir->{keywords},
      description => $parentDir->{description}
    );
  }
  ...
  $where
    ...
    ? ' `rating`,'
    : '')
    . "`$params{order_field}` $params{order_desc} "
    . ($params{order_field} ne 'ID' ? ",`ID` desc" : "");
  if ($params{limit}) {
...
sub text_main_item {
  ...
  return $self->render_not_found
    unless my $text
    = $self->dbi->query(
    "SELECT *, DATE_FORMAT(updated_at,'%a, %d %b %Y %T') AS updated_at_rfc822 FROM `texts_main_"
      . $self->lang
      . "` WHERE $where")->hash;
...
sub text_list_item {
  my $self = shift;
  my %params = (where => "`ID`='" . $self->stash('ID') . "'", @_);
...
#	}
  ...
  return $self->render_not_found unless my $item = $self->dbi->query(
    qq/
			...
			FROM `$table`
			WHERE `alias`='$alias' $where LIMIT 1/
  )->hash;

  if ($self->stash->{alias}) {
    if (
      my $textSection = $self->dbi->query(
            "SELECT `name`,`title` FROM `texts_main_"
          . $self->lang
          . "` WHERE `alias`='"
          . $self->stash->{alias}
          . "' AND `viewtext`='1'"
