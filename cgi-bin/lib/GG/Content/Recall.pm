...
sub list{
	my $self = shift;
	my %params = (
		limit 	=> $self->get_var(name => 'recall_limit', controller => 'recall') || 10,
	...
	my $items = $self->dbi->query("SELECT * FROM `data_recall` WHERE $where")->hashes;

	return $self->render_not_found unless my $item = $self->dbi->query("SELECT * FROM `texts_main_".$self->lang."` WHERE `alias`='recall' AND `viewtext`=1")->hash;
	...
	if($self->req->method eq 'POST'){
		...
		if(!keys %{ $self->flash->{'recall_form_errors'} }){
			...
			$self->dbi->insert_hash('data_recall', {
			...
			my $email_body = 	$self->render_mail(
				template	=> "Recall/_admin"
			);

			eval{
				$self->mail(
					to      => $self->get_var(name => 'email_admin', controller => 'global', raw => 1),,
					subject => 'Получен новый отзыв с сайта '.$self->site_name,
					data    => $email_body,
				);
			};
