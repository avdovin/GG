...
sub images_list {
  ...
  if (my $dirAlias = $self->stash->{dir_alias}) {
    ...
    return $self->render_not_found
      unless $gallery
      = $self->dbi->query(
      "SELECT * FROM `$dbTable` WHERE `alias`='$dirAlias' AND `dir`='1' AND `viewimg`='1' "
      )->hash;
...
sub images_item {
  ...
  my $order = "ID";
  $order = "rating"
    if ($self->dbi->exists_keys(from => $table, lkey => 'rating'));

  my $image = $self->dbi->query(
    qq/
				SELECT *
				FROM `$table`
  ...
  $image->{index_after} = $self->get_index_after(
    from  => $table,
    index => $ID,
    ring  => $ring,
    where => $where,
  ...
  $image->{index_befor} = $self->get_index_befor(
    from  => $table,
    index => $ID,
    ring  => $ring,
    where => $where,
  ...
  $self->render(item => $image, template => $template);
}
1;
